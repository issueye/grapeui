  ...
  <div class="flex flex-row w-full bg-white shadow">
    <div v-for="(item, index) in navBar" :key="index" class="flex flex-row h-8 items-center first:ml-1">
      ...
      <div v-else
        ...
        @click="onMenuItemClick(item)">
        {{ item.title }}
        <Icon icon="carbon:close-outline" @click.stop="onMenuRemoveClick(item, 1)" v-if="item.index !== dashboard.index"
...
const onMenuItemClick = (item) => {
  ...
  // console.log('item:click -> index', item.index);
